(function () {
  ...
  ********************************************** */
  ...
  Compiler.prototype._initRenderer = function _initRenderer () {
    ...
    origin.link = linkCompiler({ renderer: renderer, router: router, linkTarget: linkTarget, compilerClass: _self });
  ...
  function scrollIntoView(path, id) {
    ...
    var section = find('#' + css_escape(id));
  ...
  var INCLUDE_COMPONENT = '__include__';
  var cached$1 = {};
  ...
  function expandInclude(tokens) {
    ...
    tokens.forEach(function (e) {
      if (e.type === INCLUDE_COMPONENT && e.components) {
        e.components.forEach(function (c) { return expandedTokens.push(c); });
      } else {
